...
app.prepare().then(() => {
  ...
  wss.on("connection", (ws) => {
    ...
    ws.on("message", (message) => {
      ...
      // Broadcast the message to all connected clients - basic santize .tostring and match
      wss.clients.forEach((client) => {
        if (client !== ws && client.readyState === WebSocket.OPEN) {
          if (message.toString().startsWith("startrun_")) {
            console.log(
              `sending start run to listening clinets, hopefully our local program gets it....`
            );
